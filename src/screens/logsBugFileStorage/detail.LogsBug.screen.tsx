...
const DetailLogsBugScreen = () => {
  ...
  const [device] = useState<any>(
    JSON.parse(route?.params?.item?.device || '{}'),
  );
  ...
  const renderAPI = useCallback((item: string) => {
    ...
    return (
      <View key={apiObject?.endpoint} style={styles.viewAPI}>
        ...
        <BText variant={'bodyLarge'} style={styles.marginTop16}>
          {'Endpoint:'}
        </BText>
        <BText
          variant={'bodyLarge'}
        ...
        {apiObject?.error && (
          <>
            ...
            <BText
              variant={'bodyLarge'}
              onPress={() => Clipboard.setString(apiObject?.error || '')}>
        ...
        {apiObject?.messageError && (
          <>
            ...
            <BText
              variant={'bodyLarge'}
              onPress={() =>
                Clipboard.setString(apiObject?.messageError || '')
              }>
    ...
    <ScrollView
      ...
      <View
        ...
        <Pressable
          ...
          style={{
            backgroundColor:
              item?.status === FirebaseConstant.ETypeOfBug.New
      ...
      <BText onPress={() => Clipboard.setString(item?.error || '')}>
        {item?.error || 'Không có'}
      </BText>
      ...
      {item?.screenShot ? (
        <>
          ...
          <Pressable
            onPress={() => Clipboard.setString(item?.screenShot || '')}>
            <Image
      ...
      <BText onPress={() => Clipboard.setString(item?.user || '')}>
        {item?.user || 'Không có'}
      </BText>
      ...
      <BText onPress={() => Clipboard.setString(item?.channelId || '')}>
